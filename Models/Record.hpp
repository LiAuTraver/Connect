...
struct Record {
	...
	qint64 elapsedMilliseconds; /// time
		...
		_NODISCARD CONNECT_INLINE bool operator==(const Record &rhs)
		const noexcept(noexcept(this->name == rhs.name)) {
			return name == rhs.name &&
			       score == rhs.score &&
			       elapsedMilliseconds == rhs.elapsedMilliseconds;
		}

		_NODISCARD CONNECT_INLINE bool operator!=(const Record &rhs)
		const noexcept(noexcept(!this->operator==(rhs))) {
